package com.InAction.X.x21InAction.add_task.view;
...
public class TempTasksAdapter extends RecyclerView.Adapter<TempTasksAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull TempTasksAdapter.ViewHolder holder, int position) {
        ...
        holder.taskNameTextView.setText(tempTaskList.get(position).getName());
        holder.taskDayTextView.setText(tempTaskList.get(position).getCalendar().get(Calendar.HOUR) + " : " + tempTaskList.get(position).getCalendar().get(Calendar.MINUTE));
    }
